...
class IBizForm extends IBizControl {
    ...
    public load2(opt: any = {}): void {
        ...
        this.load(arg).subscribe((action) => {
            ...
            this.fire(IBizForm.FORMLOADED, this);
    ...
    public regField(field: any): void {
        ...
        if (field) {
            field.on(IBizFormItem.VALUECHANGED).subscribe((data: any = {}) => {
                if (this.ignoreformfieldchange) {
                    return;
                }
                this.formDirty = true;
                this.fire(IBizForm.FORMFIELDCHANGED, data);
            });
     ...
     * @memberof IBizForm
     */
    public load(opt: any = {}): Subject<any> {
        // tslint:disable-next-line:prefer-const
        let arg: any = {};
        Object.assign(arg, opt);
        const subject = new rxjs.Subject();

        this.iBizHttp.post(this.getBackendUrl(), arg).subscribe((data) => {
            if (data.ret === 0) {
                subject.next(data);
            } else {
                subject.error(data);
     ...
     * @memberof IBizForm
     */
    public submit(opt: any = {}): Subject<any> {
        // tslint:disable-next-line:prefer-const
        let arg: any = {};
        Object.assign(arg, opt);
        const subject = new rxjs.Subject();
    ...
    public updateFormItems(mode: string): void {
        ...
        // this.ignoreformfieldchange=true;
        this.load(arg).subscribe((action) => {
            this.fire(IBizForm.UPDATEFORMITEMED, action.data);
            this.setFieldAsyncConfig(action.config);
            this.setFieldState(action.state);
    ...
    public static UPDATEFORMITEMBEFORE = 'UPDATEFORMITEMBEFORE';

    public static UPDATEFORMITEMED = 'UPDATEFORMITEMED';
